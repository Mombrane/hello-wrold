...
```
┌─────────────────────────────────────────────────────────────┐
│  Layer 3: Memories（本地 rollout 管道）                       │
│  完全自动 · 逐会话提取 → 合并 → MEMORY.md + memory_summary   │
│  无需 agent 主动调用工具                                      │
├─────────────────────────────────────────────────────────────┤
│  Layer 2: Hindsight（远程 API 服务）                          │
│  Vectorize 托管 · retain/recall/reflect 三工具               │
│  Mental Model · 防反馈循环 · 标签作用域                       │
├─────────────────────────────────────────────────────────────┤
│  Layer 1: Mnemosyne（本地 SQLite 引擎）                       │
│  Beam 双层记忆 · FTS5 + ONNX 向量 · 睡眠巩固 · 遗忘曲线      │
│  事实版本追踪 · 注解系统 · 时间线                             │
└─────────────────────────────────────────────────────────────┘
```
