...
![RMI vs 手写 RPC 框架对比](assets/rpc/rpc-rmi-vs-custom.png)
...
说实话，Dubbo 的功能确实强大，但对中小团队来说可能有些「杀鸡用牛刀」。我见过不少团队引入 Dubbo 后，被其复杂的配置和概念（服务分组、版本管理、集群容错策略）搞得晕头转向，最后反而降低了开发效率。**如果你的团队规模 <20 人，服务数量 <50 个，Spring Cloud + OpenFeign 可能是更务实的选择。**

### gRPC 的「生态碎片化」

gRPC 虽然跨语言能力出色，但每个语言的实现质量和文档完善度差异巨大。Go 和 Java 的 gRPC 库成熟度很高，但 Python、Node.js 的 gRPC 库在错误处理、连接管理等方面仍有坑。更麻烦的是，gRPC 的调试工具链远不如 REST API 成熟——你很难用 curl 或 Postman 直接测试 gRPC 接口。

### 「手写 RPC 框架」的陷阱

面试中经常考「手写一个 RPC 框架」，但我认为这道题的真正考点不是代码实现，而是**系统设计思维**。真正有价值的思考包括：
...
这些问题在面试的 30 分钟内根本无法完整实现，但能展示你对分布式系统的理解深度。

### 我的建议

对于大多数 Java 团队，我的建议是：**先用好 Spring Cloud，等真正遇到性能瓶颈或跨语言需求时，再考虑 Dubbo 或 gRPC。** 不要为了「技术先进性」而引入复杂度。
